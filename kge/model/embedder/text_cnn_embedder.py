...
class TextCNNEmbedder(KgeEmbedder):
    def __init__(
        ...
        if self.dim < 0:
            raise ValueError ("dim was not initialized correctly")
    ...
    def process_text(self):
        ...
        # the text descriptions are ordered based on their corresponding
        # entity id
        # missing descriptions are set to empty lists
    ...
    def preprocess_text(self,data):
        ...
        for entity in entities:
            if entity in data_index:
                text = self._preprocess_text(text=data_index[entity])
            else:
                text = []
