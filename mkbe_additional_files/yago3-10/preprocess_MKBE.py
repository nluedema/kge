...
if __name__ == "__main__":
    ...
    # disregard subjects, as they have already been added
    numerical_data_filename = "../mkbe_additional_files/yago3-10/numerical_data_preprocessed"
    with open(numerical_data_filename,"r") as f:
        numerical_triples = list(map(lambda s: s.strip().split("\t"), f.readlines()))
        for t in numerical_triples:
            if t[P] not in relations:
                relations[t[P]] = rel_id
                rel_id += 1
            if t[O] in entities:
                raise ValueError("numeric entities should only appear once")

            entities[t[O]] = ent_id
            ent_id += 1
        
        print(
            f"Found {len(numerical_triples)} triples "
            f"(file: {numerical_data_filename})."
        )
    
    # add numeric triples to train triples
    # cannot do this earlier, because I want the numeric entities to have the
    # highest id's
    # if I do it earlier entities from valid or test might have a higher id
    raw["train"].extend(numerical_triples)

    #correct for numeric entities
    split_sizes["train"] = len(raw["train"])

    # it is unproblematic that the numeric entities and relations are missing from
    # entities_in_train and relations_in_train, because all the numeric entities are
    # already in the training set and all the numeric relations are not in the test
    # or valid set anyway (i.e. do not effect the without_unseen files)
