...
with open(
    "/work-ceph/nluedema/kge/mkbe_additional_files/fb15k-237/entityWords.txt"
) as f:
    data = list(
        map(lambda s: s.strip().split("\t"), f.readlines())
    )

#Already preprocessed by DKRL paper
text_data = []
for t in data:
    text_data.append(t[2].split(" "))
