    ...
    with open(os.path.join(best_model["folder"], "config.yaml"), "r") as best_model_config:
        ...
        for line in best_model_config:
            ...
            if "num_trials" in new_line:
                new_line = new_line.replace("  num_trials: 20", "  num_trials: 5")
            if "num_sobol_trials" in new_line:
                new_line = new_line.replace("  num_sobol_trials: 20", "  num_sobol_trials: 5")
            ...
            # replace bounds parameters
            if "bounds" in new_line:
                # drop bounds values (and stuff like log_scale)
                next_line = next(best_model_config)
                while "  name: " not in next_line:
                    next_line = next(best_model_config)
                # get and write current ax param
                ax_param = next_line.split("#")[0].strip().split(": ")[-1]
                next_line = next_line.replace("  name: ", "- name: ").strip("\n")
                output_file.write("\n" + next_line)
                # add fixed type
                next_line = next(best_model_config)
                if "range" in next_line:
                    output_file.write("\n" + "    type: fixed")
                else:
                    raise ValueError(f"Expected type:range instead got {next_line}")
                # add fixed value
                output_file.write("\n    value: " + str(best_model[ax_param]))
            else:
