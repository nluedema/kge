...
def mean_std_table(
        ...
        all_data = all_data[["model", "dataset", "modalities", metrics[0], metrics[1]]]
        ...
        for dataset in dataset_labels:
            if dataset == dataset_labels[-1]:
                output_file.write(f"\multicolumn{{{len(metrics)}}}{{c}}{{{dataset}}} \\\\ \n")
            else:
                output_file.write(f"\multicolumn{{{len(metrics)}}}{{c}}{{{dataset}}} & ")
        ...
        for dataset in datasets:
            for metric in range(len(metrics)):
                if dataset == datasets[-1] and metrics[metric] == metrics[-1]:
                    output_file.write(f"{metric_labels[metric]} \\\\ \n")
                else:
                    output_file.write(f"{metric_labels[metric]} & ")
        output_file.write("    \\midrule \n")

        # get data
        mean = all_data.groupby(["model", "modalities" ,"dataset"], as_index=False).mean()
        std = all_data.groupby(["model", "modalities" ,"dataset"], as_index=False).std()
        min_df = all_data.groupby(["model", "modalities" ,"dataset"], as_index=False).min()
        max_df = all_data.groupby(["model", "modalities" ,"dataset"], as_index=False).max()
        ...
        for dataset in datasets:
            for metric in range(len(metrics)):
                entry_mean = mean.loc[
                    (mean["model"] == model) &
                    (mean["dataset"] == dataset)
                ]
                entry_std = std.loc[
                    (std["model"] == model) &
                    (std["dataset"] == dataset)
                ]
                entry_min = min_df.loc[
                            (min_df["model"] == model) &
                            (min_df["dataset"] == dataset)
                        ]
                entry_max = max_df.loc[
                    (max_df["model"] == model) &
                ...
                for dataset in datasets:
                    for metric in range(len(metrics)):
                        entry_mean = mean.loc[
                            (mean["model"] == model) &
                            (mean["dataset"] == dataset) &
                            (mean["modalities"] == modality)
                        ]
                        entry_std = std.loc[
                            (std["model"] == model) &
                            (std["dataset"] == dataset) &
                            (std["modalities"] == modality)
                        ]
                        entry_min = min_df.loc[
                            (min_df["model"] == model) &
                            (min_df["dataset"] == dataset) &
                            (min_df["modalities"] == modality)
                        ]
                        entry_max = max_df.loc[
                            (max_df["model"] == model) &
                            (max_df["dataset"] == dataset) &
...
if __name__ == '__main__':
    ...
    # add scorer column
    all_trials['scorer'] = all_trials['model']
    best_trials['scorer'] = best_trials['model']

    # change model column
    all_trials['model'] = all_trials['folder'].str.extract(
        r'(complex|dkrl|literale|mkbe)'
    )
    best_trials['model'] = best_trials['folder'].str.extract(
        r'(complex|dkrl|literale|mkbe)'
    )
    ...
    all_trials['modalities'] = all_trials['folder'].apply(get_modalities)
    best_trials['modalities'] = best_trials['folder'].apply(get_modalities)

    # adjust dataset for mkbe
    all_trials['dataset'] = all_trials['dataset'].str.extract(
        r'(yago3-10|fb15k-237)'
    )
    best_trials['dataset'] = best_trials['dataset'].str.extract(
        r'(yago3-10|fb15k-237)'
    )
    ...
    model_order = ['complex', 'dkrl', 'literale', 'mkbe']
    model_labels = {'complex': 'ComplEx', 'dkrl': 'DKRL', 'literale': 'LiteralE', 'mkbe': 'MKBE'}

    # order and lebel for modaltities
